...
export default function RelevamientoEspaciosVerdes() {
  ...
  // ESTADOS
  const [espaciosPorComuna, setEspaciosPorComuna] = useState<any>({});
  ...
  const [respuestasActuales, setRespuestasActuales] = useState<any>({});
  const [ofertasCargadas, setOfertasCargadas] = useState<any[]>([]);
  ...
  return (
    <main
      className={`${montserrat.className} min-h-screen bg-[#07111F] text-slate-100 pb-20`}
    >
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <section className="bg-[#122033] rounded-[32px] p-6 md:p-8 mb-6 border border-white/5 shadow-xl">
          <div className="flex items-center gap-4 mb-8">
            <span className="w-8 h-8 rounded-full bg-slate-700 text-slate-200 flex items-center justify-center font-black text-sm">
              1
            </span>
        ...
        {espacioVerde && agente && horario && (
          <section
            ref={ofertaRef}
            className={`bg-[#122033] rounded-[32px] p-6 md:p-8 mb-6 border transition-all duration-500 shadow-xl ${
              animarSelector
          ...
          <section className="bg-[#07111F] rounded-[32px] p-6 md:p-8 mb-6 border-2 border-[#8DE2D6]/30 shadow-2xl">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6 pb-6 border-b border-white/5">
              <div className="text-center sm:text-left">
                <span className="text-[#8DE2D6] text-xs font-black uppercase tracking-widest">
                  Informacion de:
                </span>
            ...
            {/* BOTONES GRANDES DE FOTO EJEMPLO (CANIL / CALISTENIA UNIFICADOS) */}
            {(ofertaActual === "Canil" || ofertaActual === "Calistenia") && (
              <div className="w-full mb-8">
                <button
                  ...
                  className="w-full py-4 bg-[#1A2E44] hover:bg-[#233A54] border-2 border-[#334E6F] rounded-2xl font-bold text-xl text-white flex items-center justify-center gap-3 transition-all cursor-pointer shadow-md"
                >
                  📷 Foto ejemplo
                </button>
              </div>
              ...
              {preguntas[ofertaActual].map((pregunta) => (
                <div key={pregunta}>
                  {pregunta === "Observaciones" ? (
                    <div className="mt-6">
                      <label className="block text-xs font-black uppercase text-white/40 mb-2 ml-2">
                      ...
                      <textarea
                        ...
                        className="w-full rounded-2xl bg-[#122033] border border-white/10 p-5 text-white focus:border-[#8DE2D6] outline-none transition-all"
                        rows={3}
                      />
                    </div>
                  ) : (
            ...
            <button
              ...
              className="w-full mt-12 bg-[#8DE2D6] text-[#07111F] py-6 rounded-3xl text-xl font-black hover:scale-[1.02] active:scale-95 transition-all shadow-lg cursor-pointer"
            >
              Guardar Oferta ✓
            </button>
          </section>
            ...
            <div className="bg-[#122033] rounded-[30px] p-6 md:p-8">
              ...
              <div className="space-y-3 mb-6">
                {ofertasCargadas.map((o, i) => (
                  <div
                    key={o.id}
                    className="flex items-center justify-between bg-[#07111F] p-5 rounded-2xl border border-white/5 gap-4"
                  >
                    <div className="flex items-center gap-4">
                      <button
                        ...
                        className="w-10 h-10 rounded-full bg-[#8DE2D6]/10 border border-[#8DE2D6]/30 flex items-center justify-center text-[#8DE2D6] font-bold cursor-pointer"
                      >
                        ✎
                      </button>
                      <div>
              ...
              <button
                onClick={() => resetOferta("")}
                className="w-full mb-4 bg-white/5 border border-white/10 text-slate-200 py-4 rounded-[20px] font-bold cursor-pointer flex items-center justify-center gap-2"
              >
            ...
            {!mensajeExito ? (
              <>
                <div className="flex justify-center mb-8">
                  <div className="w-20 h-20 border-8 border-white/10 border-t-[#8DE2D6] rounded-full animate-spin"></div>
                </div>
              ...
              <>
                ...
                <p className="text-[#8DE2D6] font-bold uppercase tracking-widest text-xs">
        ...
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[120] w-[90%] max-w-md">
          <div className="bg-red-600 text-white p-5 rounded-3xl shadow-2xl font-black text-center flex items-center justify-center gap-4 border-4 border-red-400 animate-in slide-up-from-bottom">
            ...
            <span className="text-sm">
              ¡Atención! Te faltó completar algunas preguntas en rojo.
            </span>
          </div>
        </div>
      )}

      {/* MODAL GENERAL PARA CANIL Y CALISTENIA UNIFICADO */}
          ...
          <div className="bg-[#122033] rounded-3xl border border-white/10 p-4 max-w-lg w-full shadow-2xl relative">
            <div className="flex items-center justify-between mb-3 px-2">
              ...
              <button
                ...
                className="w-8 h-8 rounded-full bg-white/5 hover:bg-white/10 text-white flex items-center justify-center text-sm font-black cursor-pointer"
              >
                ✕
              </button>
            </div>
...
function CampoSelectConBuscador({
  ...
  return (
    <div className="flex flex-col relative" ref={containerRef}>
      <label className="text-[10px] uppercase font-black text-white/40 tracking-widest mb-2 ml-2">
        {titulo}
      </label>
      <div
        onClick={() => !disabled && setIsOpen(!isOpen)}
        className={`w-full rounded-2xl px-5 py-5 bg-[#07111F] border-2 transition-all font-bold flex justify-between items-center select-none ${
          value
      ...
      >
        ...
        <span className="opacity-30 text-xl text-white">▼</span>
      </div>
      {isOpen && (
        <div className="absolute top-[102%] left-0 w-full bg-[#121926] border-2 border-slate-700 rounded-2xl shadow-2xl z-[100] p-3 mt-1">
          <input
            ...
            className="w-full bg-[#07111F] text-white border border-white/10 rounded-xl px-4 py-3 font-bold text-sm outline-none focus:border-[#8DE2D6] mb-2"
            autoFocus
          />
    ...
    <div className="flex flex-col">
      ...
      <div className="relative">
        <select
          ...
          className={`w-full rounded-2xl px-5 py-5 bg-[#07111F] border-2 transition-all outline-none appearance-none font-bold ${
            value
              ? "border-[#8DE2D6]/40 text-white"
              : "border-white/10 text-white/40"
          } ${
          ...
          {isLoading ? (
            ...
            <>
              ...
              {opciones.map((o: string) => (
                <option key={o} value={o} className="bg-[#121926]">
                  {o}
                </option>
              ))}
            </>
        ...
        <div className="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none opacity-30 text-xl text-white">
          ▼
        </div>
      </div>
    </div>
...
function ControlCantidad({
  ...
  const ajustar = (delta: number) => {
    let actual = num === null || num === -1 ? 0 : num;
    onChange(String(Math.max(0, Math.min(50, actual + delta))));
  };

  return (
    <div
      className={`p-4 md:p-6 rounded-[24px] border-2 transition-all ${
        error
    ...
    >
      <div className="flex flex-col sm:flex-row items-center justify-between mb-4 gap-2">
        ...
        <label className="block text-sm font-bold text-center text-slate-300">
          {titulo}
        </label>
        <div className="w-auto sm:w-28 flex justify-center sm:justify-end">
          {imagenEjemplo && (
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-slate-700/60 hover:bg-slate-700 text-xs font-semibold text-[#8DE2D6] border border-white/10 transition-all cursor-pointer"
            >
              📷 Foto Ejemplo
            </button>
          )}
      ...
      <div className="flex items-center justify-between max-w-[280px] mx-auto gap-4">
        <button
          ...
          className="w-14 h-14 rounded-2xl bg-[#07111F] border border-white/10 flex items-center justify-center text-2xl font-black text-white cursor-pointer"
        >
          -
        </button>
        <div className="flex-1 text-center">
          <span
            className={`text-3xl font-black ${
              num === -1 ? "text-slate-500 text-xl" : "text-white"
            }`}
          >
      ...
      {modalAbierto && (
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#122033] rounded-3xl border border-white/10 p-4 max-w-lg w-full relative">
            <div className="flex items-center justify-between mb-3 px-2">
              <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 truncate max-w-[85%]">
                {titulo}
              </h4>
              <button
                onClick={() => setModalAbierto(false)}
                className="w-8 h-8 rounded-full bg-white/5 text-white flex items-center justify-center text-sm font-black cursor-pointer"
              >
                ✕
              </button>
            </div>
